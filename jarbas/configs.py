...
def get_llm_provider(provider: str) -> Optional[Dict[str, Any]]:
    if _config is None or "llm_providers" not in _config:
        return None
    return next((p for p in _config["llm_providers"] if p["name"] == provider), None)
